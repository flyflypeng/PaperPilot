...
当使用自动分配且配置了多个 arXiv 分类时，Daily ArXiv 会使用加权的“两阶段配额”策略：
...
2. **余量回填**：如果小众分类当天论文数量少于其保底配额，未使用的每日名额会重新分配给仍有更多匹配论文的已配置分类，直到达到 **每日最大论文数** 或没有更多可抓论文。

这样既能保留小众分类的曝光，又能在论文数量足够时尽量填满每日抓取上限。
